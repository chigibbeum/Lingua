...
{#if isOpen}
  <div class="modal-overlay" onclick={(e) => { if (e.target === e.currentTarget) handleCancel() }} onkeydown={handleKeydown} role="dialog" tabindex="-1" aria-modal="true" aria-label="Add Note">
    <div class="modal-content" role="document" tabindex="-1">
      ...
      <div class="modal-actions">
        ...
        <button
          ...
          disabled={noteType === 'vocab' ? (vocabTarget.trim().length === 0 || vocabNative.trim().length === 0) : (noteText.trim().length === 0)}
        >
          Add Note
        </button>
      </div>
