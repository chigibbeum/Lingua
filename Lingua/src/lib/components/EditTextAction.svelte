<script lang="ts">
  ...
  function handleMorphemeClick(e: Event, morphemeId: string) {
    ...
    if (editingMorphemeId === morphemeId || insertingLeft === morphemeId || insertingRight === morphemeId) {
      return
    }
  ...
  $effect(() => {
    if (editingMorphemeId) {
      const input = document.querySelector(`[data-edit-input="${editingMorphemeId}"]`) as HTMLInputElement
      if (input) {
        input.focus()
        input.select()
      }
...
<div class="edit-text-action" onclick={handleClickOutside} role="presentation" onkeydown={(e) => { if (e.key === 'Escape') handleClickOutside() }}>
        ...
        {:else}
          ...
          >
            ...
            {#if isClicked}
              <div class="popover" style="left: {popoverPosition.x}px; top: {popoverPosition.y}px;">
                <button
                  ...
                  onclick={(e) => { e.stopPropagation(); handleInsertLeft(morpheme.id) }}
                ...
                <button
                  type="button"
                  class="popover-button"
                  onclick={(e) => { e.stopPropagation(); handleEdit(morpheme.id) }}
            ...
            <button
              type="button"
              class="popover-button"
              onclick={(e) => { e.stopPropagation(); handleInsertRight(morpheme.id) }}
