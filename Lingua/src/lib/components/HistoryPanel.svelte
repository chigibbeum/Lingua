<script lang="ts">
  ...
  function filteredEntries(): HistoryEntry[] {
    const tabFiltered =
      activeTab === 'all' ? entries : entries.filter((e) => e.scope === activeTab)
...
<style>
  ...
  .entry .scope[data-scope="parse"] { color: #76abae; }
  .entry .scope[data-scope="flashcard"] { color: #b39ddb; }
  .entry .time { opacity: 0.9; }
  .entry .title {
    margin-top: 0.2rem;
    font-weight: 600;
  }
