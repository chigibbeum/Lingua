...
import { doc, setDoc, runTransaction, getDoc, serverTimestamp, type Firestore } from 'firebase/firestore'
...
export async function claimUsername(raw: string): Promise<void> {
  ...
  await runTransaction(db as Firestore, async (tx) => {
    ...
    tx.set(usernameRef, { ownerUid: currentUser.uid, createdAt: Date.now() })
...
export async function setProfileDetails(details: { firstName?: string; lastName?: string }): Promise<void> {
  const currentUser = auth.currentUser
  if (!currentUser) throw new Error('No user signed in.')
