<script lang="ts">
  ...
  import { loginWithEmailPassword, signupWithEmailPassword, loginWithGoogle, isLoggedIn } from './stores/auth'
...
<main class="login-page">
  <section class="login-card" aria-labelledby="login-title">
    ...
    <p class="subtitle">{authMode === 'signup' ? 'Sign up to get started' : 'Sign in to continue'}</p>

    {#if errorMessage}
      <div class="error" role="alert">{errorMessage}</div>
    {/if}

    <form class="login-form" onsubmit={authMode === 'signup' ? handleEmailSignup : handleEmailLogin}>
      ...
      {#if authMode === 'signup'}
        <button class="google signup" type="button" onclick={() => switchMode('signin')} disabled={isSubmitting}>
          Have an account? Sign in
        </button>
      {:else}
        <button class="google signup" type="button" onclick={() => switchMode('signup')} disabled={isSubmitting}>
          Create an account
        </button>
      {/if}
    </form>
