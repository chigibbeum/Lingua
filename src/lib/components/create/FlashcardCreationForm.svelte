<script lang="ts">
  ...
  let {
    ...
    onSubmit?: (_data: SubmitPayload) => void
    onImport?: () => void
  } = $props()
  ...
  // Right column
  let partOfSpeech = $state<
    'noun' | 'verb' | 'adjective' | 'adverb' | 'phrase' | 'grammar' | 'other'
  >('noun')
  ...
  function loadSuggestions() {
    try {
      ...
      console.warn('Failed to load flashcard suggestions', error)
    }
  }
  ...
  function saveRecents(kind: 'tag' | 'deck', value: string) {
    ...
    try {
      ...
      const set = new SvelteSet<string>(Array.isArray(arr) ? arr : [])
      set.delete(value)
      set.add(value)
      ...
      console.warn(`Failed to store recent ${kind}`, error)
    }
  }
  ...
  const filteredDeckSuggestions = $derived(
    deckInput.trim()
    ...
    <div class="col col-right">
      ...
      <div class="row">
            ...
            {#each selectedTags as tag, i (tag)}
              <span class="pill"
                >{tag}<button
                  ...
                  onclick={() => (selectedTags = removeAt(selectedTags, i))}>×</button
                ></span
              >
            {/each}
          </div>
        ...
        {#if filteredTagSuggestions.length}
          <div class="sugs">
            ...
            <div class="sugs-list">
              {#each filteredTagSuggestions as s (s)}
                <button
                  type="button"
                  class="sug"
                  onclick={() => {
                    selectedTags = addUnique(selectedTags, s)
                    saveRecents('tag', s)
                  }}>{s}</button
                >
              {/each}
      ...
      <div class="row">
        ...
        <input
          ...
          placeholder="Type a deck and press Enter"
        />
        <div class="hint" aria-live="polite">
          {#if deckInput}
        ...
        {#if selectedDecks.length}
          <div class="selected-tags">
            {#each selectedDecks as deck, i (deck)}
              <span class="pill"
                >{deck}<button
                  type="button"
                  class="remove"
                  aria-label={`Remove ${deck}`}
                  onclick={() => (selectedDecks = removeAt(selectedDecks, i))}>×</button
                ></span
              >
            {/each}
          </div>
        ...
        {#if filteredDeckSuggestions.length}
          <div class="sugs">
            <div class="sugs-title">Suggested</div>
            <div class="sugs-list">
              {#each filteredDeckSuggestions as s (s)}
                <button
                  type="button"
                  class="sug"
                  onclick={() => {
                    selectedDecks = addUnique(selectedDecks, s)
                    saveRecents('deck', s)
                  }}>{s}</button
                >
              {/each}
...
<style>
  ...
  .sug:hover,
  .sug:focus-visible {
    background: rgba(65, 87, 128, 0.25);
    outline: none;
  }
