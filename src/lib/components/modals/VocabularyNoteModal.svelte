<script lang="ts">
  ...
  let {
    ...
    onNoteAdded?: (
      _morphemeId: string,
      _payload: { type: 'vocab'; target: string; native: string; pos?: PosTag; tags?: string[] }
    ) => void
  } = $props()
  ...
  // Comma-separated tag input (e.g., "food, restaurant")
  let vocabTags = $state('')
...
{#if isOpen}
  <div
    ...
    tabindex="-1"
    aria-modal="true"
    aria-label="Add Vocabulary Note"
  >
    <div class="modal-content" role="document" tabindex="-1">
      ...
      <div class="form-group">
        ...
        <label class="label" for="vocab-pos" style="margin-top: 0.75rem;"
        ...
        <label class="label" for="vocab-tags" style="margin-top: 0.75rem;">Tags (optional)</label>
        <input
          id="vocab-tags"
          type="text"
          bind:value={vocabTags}
          class="input"
          placeholder="e.g., food, restaurant, travel"
        />
      </div>
      ...
      <div class="modal-actions">
        ...
        <button
          ...
          disabled={vocabTarget.trim().length === 0 || vocabNative.trim().length === 0}
          >Add note</button
        >
      </div>
    </div>
