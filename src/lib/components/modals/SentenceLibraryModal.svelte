<script lang="ts">
  ...
  import CircleRadioIcon from '$lib/icons/CircleRadioIcon.svelte'
  ...
  import {
    ...
    type SentenceDoc,
    type SentenceNote,
  } from '$lib/services/sentenceService'
  ...
  let vocabNoteCount = $derived(notes.filter(n => n.type === 'vocab').length)
  let grammarNoteCount = $derived(notes.filter(n => n.type === 'grammar').length)
...
{#if open}
  ...
  >
    ...
    >
      ...
      {#if !isLoading && !error && sentences.length > 0}
        <div class="summary-stats" role="status" aria-label="Library summary">
          <span class="stat">
            <strong>{sentences.length}</strong> sentence{sentences.length === 1 ? '' : 's'} saved
          </span>
          {#if selected && (vocabNoteCount > 0 || grammarNoteCount > 0)}
            <span class="stat-divider">•</span>
            <span class="stat">
              Selected: <strong>{vocabNoteCount}</strong> vocab, <strong>{grammarNoteCount}</strong>
              grammar note{grammarNoteCount === 1 ? '' : 's'}
            </span>
          {/if}
        </div>
      {/if}
              ...
              <div class="selected-wrapper">
                ...
                {#if isAddingNote}
                  <div class="add-note-form">
                    <div class="note-type-tabs" role="radiogroup" aria-label="Note type">
                      <button
                        type="button"
                        class="radio-option"
                        role="radio"
                      ...
                      >
                        <CircleRadioIcon
                          size={16}
                          stroke="#76abae"
                          fill={noteType === 'vocab' ? '#76abae' : 'none'}
                          strokeWidth={1.5}
                          ariaLabel="Vocab option"
                        />
                        <span>Vocab</span>
                      </button>
                      <button
                      ...
                      >
                        <CircleRadioIcon
                          size={16}
                          stroke="#76abae"
                          fill={noteType === 'grammar' ? '#76abae' : 'none'}
                          strokeWidth={1.5}
                          ariaLabel="Grammar option"
                        />
                        <span>Grammar</span>
                      </button>
                    </div>
