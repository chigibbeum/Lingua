...
let cards: Flashcard[] = $state([])
  ...
  let activeToolbarAction = $state<string | null>('add-flashcard')
  ...
  async function loadCards() {
    ...
    try {
      ...
      error =
        caught instanceof Error ? caught.message : 'Failed to load flashcards'
      cards = []
    } finally {
      isLoading = false
    }
...
<Toolbar
  ...
  onActionClick={action => {
    ...
    // Future actions like "filter" or "save" for flashcards
