...
let activeToolbarAction = $state<string | null>('new-text')
...
let showSentenceLibrary = $state(false)
 ...
 mode: 'idle', current: null 
  ...
  $effect(() => {
    const unsubscribe = sessionStore.subscribe(state => {
      ...
      if (
        state.mode === 'parsing' &&
        (state.current?.morphemes?.length ?? 0) > 0 &&
        currentAction === 'add-new'
      ) {
        currentAction = 'parse'
        activeToolbarAction = 'parse'
      }
      ...
      const hasMorphemes = Boolean(
        state.current?.morphemes?.length && state.current.morphemes.length > 0
      )
  ...
  function handleActionChange(action: ParsingAction) {
    ...
    activeToolbarAction =
      action === 'add-new' ? 'new-text' : action === 'parse' ? 'parse' : 'edit'
 ...
 snippet: session.current.sentence 
} : {
}),
            ...(session.current?.id ? {
 relatedId: session.current.id 
} : {
}),
          })
        }
          ...
          saveSentenceWithNotes(text, noteInputs)
            .then(() => {
              ...
              historyStore.add({
                ...
                ...(count ? {
 description: `Saved ${count} note${count === 1 ? '' : 's'}` 
} : {
}),
                snippet: text,
                ...(session.current?.id ? {
