...
const cardsSorted = derived([flashcardsStore, tagsStore, searchQuery], ([$cards, $tags, q]) => {
...
query(collection(db, 'flashcards'), where('userId', '==', currentUserId))
...
// 5. Search flashcards
cards.filter(c => c.targetVocabulary.includes(query) || c.nativeTranslation.includes(query))
```
